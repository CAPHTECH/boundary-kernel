...
> **ステータス: 実験的 (v0.4.0)。** スキーマはまだ固定していません。本番運用を前提にした保証はありません。v0.2 で `rbk.decision` に破壊的変更が入り(下記)、v0.3 で同一性に `identity.policy_digest` が必須になりました。後者も破壊的変更なので、**スキーマは `rbk.decision.v3` として切り直しています** — 公開済みの識別子のまま内容を変えるのは、このカーネルが policy について警告している「同じラベルで内容が違う」状態そのものだからです([理由](docs/00_design.md#なぜ-v3-を切ったか--自分の規律を自分に適用する))。

> ⚠️ **v0.4 の訂正 — 読めない入力を通していた(fail-open)。** v0.3.0 まで、`decide()` は入力を実行時に検証していませんでした。宣言された enum の外にある値は順序比較の `indexOf` が `-1` を返すため比較が黙って通り、**理由欄が空のまま `auto_apply` と `basis_complete: true` に合成されていました。**大文字小文字の違いで足りました — `"critical"` は `human_required`、`"CRITICAL"` は `auto_apply`。同じ形で倒れる箇所が**8つ**あり、`action.reversibility` は enum 外で例外を投げていました。**人間を通さず適用してよいかを決める部品が、読めなかった入力を「安全」と報告していたことになります。**v0.4 は定義域外の値を、それを読む factor の `human_required` + `basis_complete: false` + 非空の理由に変えます。**これはホストに対する破壊的変更です**(これまで `auto_apply` を受け取っていた入力が `human_required` を返します)。公開スキーマ `rbk.request.v1` / `rbk.policy.v1` / `rbk.decision.v3` は**変更していません** — enum はもともとそこに宣言されており、通らなくなる入力は元からスキーマ違反だったためです。`KERNEL_VERSION` は `decision_id` の入力なので、**同じ request でも v0.3.0 とは異なる `decision_id` になります**(意図した挙動 — 別のカーネルは別の境界を引きうる)。
...
いずれかの factor が `incomplete` 信号を1つでも出したら `basis_complete = false` になります。`outcome` が `human_required` でも、です。`outcome == incomplete` は「`basis_complete == false` かつ `human_required` が無い」場合と厳密に一致し、**基盤の欠損は outcome に関わらず必ず記録されます**。欠損を記録した decision は、行き先が `human_required` であっても `routing.required_evidence_modes` を伴います。
